...
assert.match(indexHtml, /id="speedup-inventory-list"/u, "所持スピードアップの一覧がありません");
assert.match(indexHtml, /id="speedup-inventory-editor"[^>]*hidden/u, "所持スピードアップの共通編集欄がありません");
assert.match(appSource, /function saveSpeedupInventoryEntry\(\)/u, "所持スピードアップの追加・編集処理がありません");
...
assert.equal([...research.nodes.values()].reduce((sum, node) => sum + node.levels.size, 0), 3143, "研究の詳細レベル数が一致しません");
