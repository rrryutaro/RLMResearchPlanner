...
- 研究計画だけを指示データとして書き出し、他の利用者の現在レベルを維持したまま不足分を取り込み
- 計画内の各研究を1件ずつ完了として反映
...
- 課金パックを入力・保存一覧・比較に分け、目的別のジェム換算とポイントで比較
- 通常・研究・訓練・建設・治療・召喚・月晶工房の各スピードアップと任意アイテムを手動入力
- 課金比較データを書き出し、プレイヤー設定を置き換えずに他の利用者のデータを追加
- 日本語・英語UIと、利用者が編集・追加できる翻訳ファイル
- 従来のデスクトップ表示を維持したまま、ヘルプの「画面デザイン」からPWA版に合わせたモバイル風配色へ切り替え
- デスクトップ表示の研究ツリー背景にもグリッドを表示
- Windows標準タイトルバー付き画面では、初期化途中にネイティブ画面を作らず、最終配色の不透明面と全UIを構築してから表示する起動方式へ変更
...
モバイルPWA版v0.0.14では、従来の研究・建設計画に加え、研究指示データの受け渡し、利用者が編集できる翻訳ファイル、課金パックの入力・保存・比較・共有を利用できます。課金比較はプレイヤー設定を置き換えず、必要なデータだけを追加します。
